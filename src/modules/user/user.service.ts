...
export class UserService {
  ...
  constructor(
    ...
    private bdRepo: Repository<BankDetailsEntity>,
  ...
  async updateBankDetails(
    ...
    try {
      ...
      if (exist.accountNumber != req.accountNumber) {
        const acctInUse = await this.bdRepo.find({
      ...
      if (exist.accountName != req.accountName) {
        const acctInUse = await this.bdRepo.find({
          where: { accountName: req.accountName },
        });
        if (acctInUse) {
          return {
            status: 400,
  ...
  async cardExist(userId, cardSignature): Promise<CardEntity> {
    return await this.cardRepo.findOne({
      where: { userId, signature: cardSignature },
    });
  }
  ...
  async getStats(user: UserEntity): Promise<any> {
    try {
      ...
      const { totalwithdrawals } = await await this.withdrawalRepo
  ...
  async updateSetting(
    ...
    try {
      const card = await this.cardRepo.findOne({
        where: { userId: user.id, id: payload.cardId },
      });
      if (!card) {
        return clientFeedback({
          status: 400,
      ...
      const set = await this.userSetRepo.findOne({
  ...
  async getSetting(user: UserEntity): Promise<IClientReturnObject> {
    const setting = await this.userSetRepo.findOne({
      where: { userId: user.id },
    });
